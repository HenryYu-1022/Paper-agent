...
| macOS 开机登录后自动启动 watcher | `install_or_update_launch_agent.sh` | 仓库根目录 | 不会 | `paper_agent_watch_supervisor.sh` + `watch_folder_resilient.py` |
| Windows 登录后自动启动 watcher | `install_or_update_watch_task.ps1` | 仓库根目录 | 不会 | `paper_agent_watch_supervisor.ps1` + `watch_folder_resilient.py` |

可以直接这么理解：

- `run_once.py` 是手动模式的主入口。
- `watch_folder_resilient.py` 是最核心的常驻监听进程。
- `install_or_update_launch_agent.sh` 和 `install_or_update_watch_task.ps1` 只是安装器，不是守护进程本体。
...
- `install_or_update_launch_agent.sh`：一次性安装 / 更新脚本
- `remove_launch_agent.sh`：一次性卸载脚本
...
- `install_or_update_watch_task.ps1`：一次性安装 / 更新脚本
- `remove_watch_task.ps1`：一次性卸载脚本
